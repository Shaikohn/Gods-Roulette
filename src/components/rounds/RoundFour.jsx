...
import MobileRoundFour from './mobile/MobileRoundFour'

const RoundFour = ({players, setPlayers, setRound}) => {
  ...
  const addPreBalance = () => {
    if(input !== '') {
      ...
      updatedPlayers[turn].rounds[3].preBalance = input;
  ...
  const addPostBalance = () => {
    if(input !== '') {
      const updatedPlayers = [...players];
      updatedPlayers[turn].rounds[3].postBalance = input;
      const difference = updatedPlayers[turn].rounds[3].postBalance - updatedPlayers[turn].rounds[3].preBalance;
      const balance = (difference / updatedPlayers[turn].rounds[3].preBalance)
      updatedPlayers[turn].rounds[3].balance = parseFloat(balance.toFixed(3))
  ...
  return (
    <div>
      <div className='mobile-only' style={{textAlign: 'center'}}>
        <MobileRoundFour players={players} setPlayers={setPlayers} setRound={setRound} />
      </div>
    <div className='roundFourContainer pc-only'>
      ...
      <Modals isOpenModal={isOpenGameModal} closeModal={closeGameModal}>
      {
        ...
        }
        {
          ...
          <div>
            {
              players[turn].rounds[3].preBalance === 0 && leaderboard === false ?
              <div>
