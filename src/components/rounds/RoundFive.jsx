...
import MobileRoundFive from './mobile/MobileRoundFive'

const RoundFive = ({players, setPlayers, setRound}) => {
  ...
  return (
    <div>
      <div className='mobile-only' style={{textAlign: 'center'}}>
        <MobileRoundFive players={players} setPlayers={setPlayers} setRound={setRound} />
      </div>
    <div className='roundFiveContainer pc-only'>
      ...
      <Modals isOpenModal={isOpenGameModal} closeModal={closeGameModal}>
      {
        ...
        }
        {
          ...
          {
            turn === max ?
            <button className='spinButton' onClick={() => finishGame()}>SIGUIENTE</button> : <button className='spinButton' onClick={() => nextTurn()}>SIGUIENTE</button>
          }
          </div>
          )
          : ''
        }
        {
          ...
          <div className='finishedContainer'>
            ...
            <div className='trophyContainer'>
              <img className='trophyImage' src={trophy} alt='' />
              <p className='winner'> {players[0].name} </p>
            </div>
            <p> 2- {players[1].name}</p>
            {players.length === 3 ? <p> 3- {players[2].name}</p> : ''}
            <button className='spinButton' onClick={() => reloadButton()}>VOLVER AL INICIO</button>
            <button className='spinButton' style={{marginLeft: '50px'}}><a href='https://deltabet.xyz/' target='blank'>MAS JUEGOS</a></button>
          </div> : ''
        }
      </Modals>
    </div>
