...
import MobileRoundTwo from './mobile/MobileRoundTwo'

const RoundTwo = ({players, setPlayers, setRound}) => {
  ...
  const addPreBalance = () => {
    if(input !== '') {
      ...
      updatedPlayers[turn].rounds[1].preBalance = input;
  ...
  const addPostBalance = () => {
    if(input !== '') {
      const updatedPlayers = [...players];
      updatedPlayers[turn].rounds[1].postBalance = input;
      const difference = updatedPlayers[turn].rounds[1].postBalance - updatedPlayers[turn].rounds[1].preBalance;
      const balance = (difference / updatedPlayers[turn].rounds[1].preBalance)
      updatedPlayers[turn].rounds[1].balance = parseFloat(balance.toFixed(3))
  ...
  return (
    <div>
      <div className='mobile-only' style={{textAlign: 'center'}}>
        <MobileRoundTwo players={players} setPlayers={setPlayers} setRound={setRound} />
      </div>
    <div className='roundTwoContainer pc-only'>
      ...
      <Modals isOpenModal={isOpenGameModal} closeModal={closeGameModal}>
      {
        ...
        }
        {
          ...
          <div>
            {
              players[turn].rounds[1].preBalance === 0 && leaderboard === false ?
              <div>
